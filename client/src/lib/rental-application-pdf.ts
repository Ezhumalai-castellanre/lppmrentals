...
export class RentalApplicationPDF {
  ...
  private addBankTable(banks: any[], title = "Bank Records") {
  ...
  private addApplicantInfo(applicant: any) {
  ...
  private addCoApplicantInfo(coApplicant: any, index: number) {
  ...
  private addGuarantorInfo(guarantor: any, index: number) {
  ...
  private addOccupants(occupants: any[]) {
    ...
    occupants.forEach((o: any, idx: number) => {
      this.addKeyValueTable(o, [], `Occupant ${idx + 1} Information`);
    });
  }

  private addSignatures(data: any) {
    ...
    const rows: string[][] = [
      [`Primary Applicant: ${data.applicant?.name || 'N/A'} - Signature: ______________________________`],
    ];
    
    if (data.coApplicants?.length) {
      data.coApplicants.forEach((co: any, i: number) =>
        rows.push([
    ...
    if (data.guarantors?.length) {
      data.guarantors.forEach((g: any, i: number) =>
        rows.push([
  ...
  generate(data: any) {
    ...
    if (formData.coApplicants && formData.coApplicants.length > 0) {
      ...
      formData.coApplicants.forEach((co: any, idx: number) => {
        console.log(`Processing co-applicant ${idx + 1}:`, co);
        this.addCoApplicantInfo(co, idx);
      });
    } else {
      console.log('No co-applicants found or empty array');
      this.addSectionTitle("Co-Applicants Information");
      this.doc.setFontSize(10);
      this.doc.setFont("helvetica", "normal");
      this.doc.setTextColor(128, 128, 128);
      this.doc.text("No co-applicants provided", 20, this.currentY);
      this.currentY += 20;
    }
