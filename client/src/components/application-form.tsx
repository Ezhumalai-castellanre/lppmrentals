...
export function ApplicationForm() {
  ...
  const onSubmit = async (data: ApplicationFormData) => {
    ...
    try {
      ...
      try {
        ...
        // 🧪 TEST WEBHOOK STRUCTURE - Verify exact format
        const testWebhookStructure = {
          reference_id: referenceId,
          application_id: applicationId,
          form_data: {
            // Application Info
        ...
        console.log('🧪 TEST WEBHOOK STRUCTURE - EXACT FORMAT:');
        console.log(JSON.stringify(testWebhookStructure, null, 2));
        
        // Verify all required fields are present
        const requiredFields = [
          'buildingAddress', 'apartmentNumber', 'moveInDate', 'monthlyRent', 'apartmentType', 'howDidYouHear',
          'applicantName', 'applicantDob', 'applicantSsn', 'applicantPhone', 'applicantEmail',
          'applicantLicense', 'applicantLicenseState', 'applicantAddress', 'applicantCity', 'applicantState', 'applicantZip',
          'applicantLengthAtAddressYears', 'applicantLengthAtAddressMonths', 'applicantLandlordName',
          'applicantLandlordAddressLine1', 'applicantLandlordCity', 'applicantLandlordState', 'applicantLandlordZipCode',
          'applicantLandlordPhone', 'applicantLandlordEmail', 'applicantCurrentRent', 'applicantReasonForMoving',
          'applicantEmploymentType', 'applicantEmployerName', 'applicantEmployerAddress', 'applicantEmployerCity',
          'applicantEmployerState', 'applicantEmployerZip', 'applicantEmployerPhone', 'applicantPosition',
          'applicantStartDate', 'applicantSalary', 'applicantBankRecords',
          'otherOccupants', 'landlordTenantLegalAction', 'brokenLease', 'signatures', 'signatureTimestamps',
          'documents', 'encryptedDocuments', 'hasCoApplicant', 'hasGuarantor'
        ];
        
        const missingFields = requiredFields.filter(field => {
          const value = (testWebhookStructure.form_data as any)[field];
        ...
        console.log('- Application Info:', Object.keys(testWebhookStructure.form_data).filter(k => k.includes('building') || k.includes('apartment') || k.includes('moveIn') || k.includes('monthly') || k.includes('howDid')).length, 'fields');
        console.log('- Primary Applicant:', Object.keys(testWebhookStructure.form_data).filter(k => k.startsWith('applicant') && !k.includes('Bank')).length, 'fields');
        console.log('- Applicant Bank Records:', testWebhookStructure.form_data.applicantBankRecords?.length || 0, 'records');
        console.log('- Co-Applicant:', Object.keys(testWebhookStructure.form_data).filter(k => k.startsWith('coApplicant')).length, 'fields');
        console.log('- Co-Applicant Bank Records:', testWebhookStructure.form_data.coApplicantBankRecords?.length || 0, 'records');
        console.log('- Guarantor:', Object.keys(testWebhookStructure.form_data).filter(k => k.startsWith('guarantor')).length, 'fields');
        console.log('- Guarantor Bank Records:', testWebhookStructure.form_data.guarantorBankRecords?.length || 0, 'records');
        console.log('- Other Occupants:', testWebhookStructure.form_data.otherOccupants?.length || 0, 'occupants');
        console.log('- Legal Questions:', Object.keys(testWebhookStructure.form_data).filter(k => k.includes('Legal') || k.includes('Lease')).length, 'fields');
        console.log('- Signatures:', Object.keys(testWebhookStructure.form_data.signatures || {}).length, 'signatures');
        console.log('- Documents:', Object.keys(testWebhookStructure.form_data.documents || {}).length, 'document types');
        console.log('- Encrypted Documents:', Object.keys(testWebhookStructure.form_data.encryptedDocuments || {}).length, 'encrypted document types');
  ...
  // 🧪 Test webhook structure function
  const testWebhookStructure = () => {
    const testData = {
      ...
      form_data: {
        ...
        applicantLandlordAddressLine1: "456 Oak Ave",
        applicantLandlordAddressLine2: "Apt 2B",
        applicantLandlordCity: "Beverly Hills",
        applicantLandlordState: "CA",
        applicantLandlordZipCode: "90211",
        applicantLandlordPhone: "+1234567891",
        applicantLandlordEmail: "jane@example.com",
        applicantCurrentRent: "2200",
        applicantReasonForMoving: "Job relocation",
        applicantEmploymentType: "Full-time",
        applicantEmployerName: "Tech Corp",
        applicantEmployerAddress: "789 Tech Blvd",
        applicantEmployerCity: "San Francisco",
        applicantEmployerState: "CA",
        applicantEmployerZip: "94105",
        applicantEmployerPhone: "+1234567892",
        applicantPosition: "Software Engineer",
        applicantStartDate: "2023-01-01T00:00:00.000Z",
        applicantSalary: "120000",
        ...
        // Co-Applicant - COMPLETE DATA
        coApplicantName: "Jane Doe",
        coApplicantRelationship: "Spouse",
        coApplicantDob: "1992-01-01T00:00:00.000Z",
        coApplicantSsn: "987-65-4321",
        coApplicantPhone: "+1234567893",
        coApplicantEmail: "jane@example.com",
        coApplicantLicense: "DL654321",
        coApplicantLicenseState: "CA",
        coApplicantAddress: "123 Main St",
        ...
        coApplicantLandlordAddressLine1: "456 Oak Ave",
        coApplicantLandlordAddressLine2: "Apt 2B",
        coApplicantLandlordCity: "Beverly Hills",
        coApplicantLandlordState: "CA",
        coApplicantLandlordZipCode: "90211",
        coApplicantLandlordPhone: "+1234567891",
        coApplicantLandlordEmail: "jane@example.com",
        coApplicantCurrentRent: "2200",
        coApplicantReasonForMoving: "Job relocation",
        coApplicantEmploymentType: "Full-time",
        coApplicantEmployerName: "Design Corp",
        coApplicantEmployerAddress: "789 Design Blvd",
        coApplicantEmployerCity: "San Francisco",
        coApplicantEmployerState: "CA",
        coApplicantEmployerZip: "94105",
        coApplicantEmployerPhone: "+1234567894",
        coApplicantPosition: "UI Designer",
        coApplicantStartDate: "2023-02-01T00:00:00.000Z",
        coApplicantSalary: "90000",
        ...
        // Guarantor - COMPLETE DATA
        guarantorName: "Bob Smith",
        guarantorRelationship: "Parent",
        guarantorDob: "1965-01-01T00:00:00.000Z",
        guarantorSsn: "111-22-3333",
        guarantorPhone: "+1234567895",
        guarantorEmail: "bob@example.com",
        guarantorAddress: "321 Pine St",
        guarantorCity: "San Diego",
        guarantorState: "CA",
        guarantorZip: "92101",
        guarantorLengthAtAddressYears: "10",
        guarantorLengthAtAddressMonths: "0",
        guarantorLandlordName: "N/A",
        guarantorLandlordAddressLine1: "N/A",
        guarantorLandlordAddressLine2: "N/A",
        guarantorLandlordCity: "N/A",
        guarantorLandlordState: "N/A",
        guarantorLandlordZipCode: "N/A",
        guarantorLandlordPhone: "N/A",
        guarantorLandlordEmail: "N/A",
        guarantorCurrentRent: "N/A",
        guarantorReasonForMoving: "N/A",
        guarantorEmploymentType: "Full-time",
        guarantorEmployerName: "Finance Corp",
        guarantorEmployerAddress: "456 Finance Ave",
        guarantorEmployerCity: "San Diego",
        guarantorEmployerState: "CA",
        guarantorEmployerZip: "92101",
        guarantorEmployerPhone: "+1234567896",
        guarantorPosition: "Manager",
        guarantorStartDate: "2015-01-01T00:00:00.000Z",
        guarantorSalary: "150000",
        ...
        otherOccupants: [
          {
            ...
            license: "N/A",
            age: "10",
            ssnDocument: "test_ssn_doc.pdf",
            ssnEncryptedDocument: "encrypted_ssn_doc.pdf"
          }
        ],
        ...
        signatureTimestamps: {
          applicant: "2025-01-15T10:30:00.000Z",
          coApplicant: "2025-01-15T10:31:00.000Z",
          guarantor: "2025-01-15T10:32:00.000Z"
        },
        
        // Documents and Encrypted Documents
        documents: {
          applicant: {
            w9_forms: ["test_w9.pdf"],
            photo_id: ["test_id.pdf"],
            social_security: ["test_ssn.pdf"],
            bank_statement: ["test_bank.pdf"],
            tax_returns: ["test_tax.pdf"],
            employment_letter: ["test_emp.pdf"],
            pay_stubs: ["test_pay.pdf"],
            credit_report: ["test_credit.pdf"]
          },
          coApplicant: {
            w9_forms: ["test_w9_co.pdf"],
            photo_id: ["test_id_co.pdf"],
            social_security: ["test_ssn_co.pdf"],
            bank_statement: ["test_bank_co.pdf"],
            tax_returns: ["test_tax_co.pdf"],
            employment_letter: ["test_emp_co.pdf"],
            pay_stubs: ["test_pay_co.pdf"],
            credit_report: ["test_credit_co.pdf"]
          },
          guarantor: {
            w9_forms: ["test_w9_guar.pdf"],
            photo_id: ["test_id_guar.pdf"],
            social_security: ["test_ssn_guar.pdf"],
            bank_statement: ["test_bank_guar.pdf"],
            tax_returns: ["test_tax_guar.pdf"],
            employment_letter: ["test_emp_guar.pdf"],
            pay_stubs: ["test_pay_guar.pdf"],
            credit_report: ["test_credit_guar.pdf"]
          }
        },
        encryptedDocuments: {
          applicant: {
            w9_forms: ["encrypted_w9.pdf"],
            photo_id: ["encrypted_id.pdf"],
            social_security: ["encrypted_ssn.pdf"],
            bank_statement: ["encrypted_bank.pdf"],
            tax_returns: ["encrypted_tax.pdf"],
            employment_letter: ["encrypted_emp.pdf"],
            pay_stubs: ["encrypted_pay.pdf"],
            credit_report: ["encrypted_credit.pdf"]
          },
          coApplicant: {
            w9_forms: ["encrypted_w9_co.pdf"],
            photo_id: ["encrypted_id_co.pdf"],
            social_security: ["encrypted_ssn_co.pdf"],
            bank_statement: ["encrypted_bank_co.pdf"],
            tax_returns: ["encrypted_tax_co.pdf"],
            employment_letter: ["encrypted_emp_co.pdf"],
            pay_stubs: ["encrypted_pay_co.pdf"],
            credit_report: ["encrypted_credit_co.pdf"]
          },
          guarantor: {
            w9_forms: ["encrypted_w9_guar.pdf"],
            photo_id: ["encrypted_id_guar.pdf"],
            social_security: ["encrypted_ssn_guar.pdf"],
            bank_statement: ["encrypted_bank_guar.pdf"],
            tax_returns: ["encrypted_tax_guar.pdf"],
            employment_letter: ["encrypted_emp_guar.pdf"],
            pay_stubs: ["encrypted_pay_guar.pdf"],
            credit_report: ["encrypted_credit_guar.pdf"]
          }
        },
      ...
      uploaded_files: {
        supporting_w9_forms: [
          {
            reference_id: "test_ref_w9",
            file_name: "test_w9.pdf",
        ...
        supporting_photo_id: [
          {
            ...
            section_name: "supporting_photo_id",
            documents: "photo_id"
          }
        ],
        supporting_social_security: [
          {
            reference_id: "test_ref_ssn",
            file_name: "test_ssn.pdf",
            section_name: "supporting_social_security",
            documents: "social_security"
          }
        ],
        supporting_bank_statement: [
          {
            reference_id: "test_ref_bank",
            file_name: "test_bank.pdf",
            section_name: "supporting_bank_statement",
            documents: "bank_statement"
          }
        ],
        supporting_tax_returns: [
          {
            reference_id: "test_ref_tax",
            file_name: "test_tax.pdf",
            section_name: "supporting_tax_returns",
            documents: "tax_returns"
          }
        ],
        supporting_employment_letter: [
          {
            reference_id: "test_ref_emp",
            file_name: "test_emp.pdf",
            section_name: "supporting_employment_letter",
            documents: "employment_letter"
          }
        ],
        supporting_pay_stubs: [
          {
            reference_id: "test_ref_pay",
            file_name: "test_pay.pdf",
            section_name: "supporting_pay_stubs",
            documents: "pay_stubs"
          }
        ],
        supporting_credit_report: [
          {
            reference_id: "test_ref_credit",
            file_name: "test_credit.pdf",
            section_name: "supporting_credit_report",
            documents: "credit_report"
          }
        ],
        coApplicant_w9_forms: [
          {
            reference_id: "test_ref_w9_co",
            file_name: "test_w9_co.pdf",
            section_name: "coApplicant_w9_forms",
            documents: "w9_forms"
          }
        ],
        coApplicant_photo_id: [
          {
            reference_id: "test_ref_id_co",
            file_name: "test_id_co.pdf",
            section_name: "coApplicant_photo_id",
            documents: "photo_id"
          }
        ],
        coApplicant_social_security: [
          {
            reference_id: "test_ref_ssn_co",
            file_name: "test_ssn_co.pdf",
            section_name: "coApplicant_social_security",
            documents: "social_security"
          }
        ],
        coApplicant_bank_statement: [
          {
            reference_id: "test_ref_bank_co",
            file_name: "test_bank_co.pdf",
            section_name: "coApplicant_bank_statement",
            documents: "bank_statement"
          }
        ],
        coApplicant_tax_returns: [
          {
            reference_id: "test_ref_tax_co",
            file_name: "test_tax_co.pdf",
            section_name: "coApplicant_tax_returns",
            documents: "tax_returns"
          }
        ],
        coApplicant_employment_letter: [
          {
            reference_id: "test_ref_emp_co",
            file_name: "test_emp_co.pdf",
            section_name: "coApplicant_employment_letter",
            documents: "employment_letter"
          }
        ],
        coApplicant_pay_stubs: [
          {
            reference_id: "test_ref_pay_co",
            file_name: "test_pay_co.pdf",
            section_name: "coApplicant_pay_stubs",
            documents: "pay_stubs"
          }
        ],
        coApplicant_credit_report: [
          {
            reference_id: "test_ref_credit_co",
            file_name: "test_credit_co.pdf",
            section_name: "coApplicant_credit_report",
            documents: "credit_report"
          }
        ],
        guarantor_w9_forms: [
          {
            reference_id: "test_ref_w9_guar",
            file_name: "test_w9_guar.pdf",
            section_name: "guarantor_w9_forms",
            documents: "w9_forms"
          }
        ],
        guarantor_photo_id: [
          {
            reference_id: "test_ref_id_guar",
            file_name: "test_id_guar.pdf",
            section_name: "guarantor_photo_id",
            documents: "photo_id"
          }
        ],
        guarantor_social_security: [
          {
            reference_id: "test_ref_ssn_guar",
            file_name: "test_ssn_guar.pdf",
            section_name: "guarantor_social_security",
            documents: "social_security"
          }
        ],
        guarantor_bank_statement: [
          {
            reference_id: "test_ref_bank_guar",
            file_name: "test_bank_guar.pdf",
            section_name: "guarantor_bank_statement",
            documents: "bank_statement"
          }
        ],
        guarantor_tax_returns: [
          {
            reference_id: "test_ref_tax_guar",
            file_name: "test_tax_guar.pdf",
            section_name: "guarantor_tax_returns",
            documents: "tax_returns"
          }
        ],
        guarantor_employment_letter: [
          {
            reference_id: "test_ref_emp_guar",
            file_name: "test_emp_guar.pdf",
            section_name: "guarantor_employment_letter",
            documents: "employment_letter"
          }
        ],
        guarantor_pay_stubs: [
          {
            reference_id: "test_ref_pay_guar",
            file_name: "test_pay_guar.pdf",
            section_name: "guarantor_pay_stubs",
            documents: "pay_stubs"
          }
        ],
        guarantor_credit_report: [
          {
            reference_id: "test_ref_credit_guar",
            file_name: "test_credit_guar.pdf",
            section_name: "guarantor_credit_report",
            documents: "credit_report"
          }
        ],
        other_occupants_identity: [
          {
            reference_id: "test_ref_occupant",
            file_name: "test_occupant.pdf",
            section_name: "other_occupants_identity",
            documents: "identity"
          }
        ]
      },
    ...
    console.log('🧪 TEST WEBHOOK STRUCTURE - COMPLETE EXAMPLE:');
    console.log(JSON.stringify(testData, null, 2));
    
    // Test webhook service with proper type
    ...
    WebhookService.sendFormDataToWebhook(
      testData.form_data,
      testData.reference_id,
      testData.application_id,
      testUploadedFiles
    ).then(result => {
      console.log('🧪 TEST WEBHOOK RESULT:', result);
      toast({
        title: "Test Webhook Sent",
        description: `Test webhook ${result.success ? 'successfully sent' : 'failed to send'}`,
      });
    });
  };
              ...
              {currentStep === STEPS.length - 1 ? (
                <div className="flex gap-2">
                  <Button
                    type="button"
                    variant="outline"
                    className="bg-yellow-500 hover:bg-yellow-600 text-white px-3 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-semibold"
                    onClick={testWebhookStructure}
                  >
